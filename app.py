...
# --- PAGE CONFIG ---
st.set_page_config(page_title="EdgeBet AI Pro", layout="wide", page_icon="🦾")
...
def fetch_sports_data(league):
    try:
        ...
        headers = {'x-rapidapi-key': api_key, 'x-rapidapi-host': 'api-sports.io'}
        ...
        return [
            {"id": 1, "match": "Sydney Swans vs Fremantle", "moneyline": 1.95, "spread": -3.5, "total": 165.5, "league": "AFL"},
            {"id": 2, "match": "Collingwood vs GWS", "moneyline": 2.10, "spread": +1.5, "total": 158.0, "league": "AFL"},
            {"id": 3, "match": "Geelong vs Adelaide", "moneyline": 1.50, "spread": -8.5, "total": 170.0, "league": "AFL"},
        ]
...
def get_props_for_game(game_id):
    return [
        {"player": "Isaac Heeney", "prop": "20+ Disposals", "odds": 1.80, "ai_prob": 0.65},
        {"player": "Andrew Brayshaw", "prop": "25+ Disposals", "odds": 2.10, "ai_prob": 0.55},
        {"player": "John Ginnane", "prop": "2+ Goals", "odds": 3.20, "ai_prob": 0.40},
        {"player": "Sydney Team", "prop": "Over 120 Points", "odds": 1.90, "ai_prob": 0.58},
    ]

# --- NAVIGATION ---
st.sidebar.title("🦾 EdgeBet Pro")
league = st.sidebar.selectbox("Select League", ["AFL", "NBA", "NFL", "NRL", "Soccer"])
...
if st.session_state.selected_game is None:
    ...
    for game in games:
        with st.container():
            ...
            if col3.button("Enter Terminal", key=f"enter_{game['id']}"):
    ...
    with c2:
        ...
        if st.button("Add Spread to SGM", key="add_sp"):
            st.session_state.sgm_ticket.append(f"{game['match']} Spread")
            st.rerun()
    with c3:
        st.metric("Total (O/U)", f"{game['total']}")
        if st.button("Add Total to SGM", key="add_tot"):
            st.session_state.sgm_ticket.append(f"{game['match']} Total")
            st.rerun()
    ...
    for p in props:
        with st.container():
            ...
            if p4.button(f"Add Prop", key=f"prop_{p['player']}"):
                st.session_state.sgm_ticket.append(f"{p['player']} {p['prop']}")
                st.rerun()
        st.markdown("---")
...
with tab1:
    ...
    st.table(pd.DataFrame([{"Game": "Syd vs Fre", "Result": "Win", "ROI": "+12%"}]))
with tab2:
